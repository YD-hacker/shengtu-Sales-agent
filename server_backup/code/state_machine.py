...
def check_qualification(state):
    ...
    if not edu or any(kw in edu for kw in ["非统招", "自考", "成人", "函授", "专升本", "高中", "中专"]):
        logger.info("   学历不合格")
        return False
    try:
        a = int(age)
...
def get_next_state(current_state, state, intent):
    """
    状态跳转 + 信任门禁
    """
    ...
    if current_state == "icebreak":
        if intent in ("newbie", "experienced", "icebreak_greet", "fee_intent"):
        ...
        if filled:
            ...
            target_state = "match_campus" if is_qualified else "reject_qualify"
        else:
            target_state = "qualify"
...
def generate_reply_instruction(old_state, new_state, state, intent, lead_score=50):
        ...
        if old_state in ("icebreak", None):
            return {
                "template": KB.get("icebreak_greet", KB.get("icebreak_normal",
                    ["你好，我是小苏，做IT人才服务6年了。你是想零基础转行学门技术，还是已经有经验想直接内推？"]))[0],
                "rules": [], "forbidden": []
            }
        # 从KB读取拉回话术
    ...
    if new_state == "icebreak":
        ...
        return {
            "template": f"按你的情况，最近的校区在{campus}，过来实训免费住宿。",
...
def _get_personalized_qualify_opening(state, lead_score):
    ...
    return "你好呀，我是小苏，做IT人才服务6年了。讲真的，转行是个大事儿，我先了解下你的情况——学历是统招大专还是本科？哪年毕业？多大了？在哪个城市？"
    ...
    if kb_objection and mode == "standard_5step":
        ...
        objection_5step = {
            ...
            "objection_learn": {
                ...
                "evidence": "讲真的，咱这边有零基础4个月转行的，从打字速度开始教，一步一步来。",
