...
_MODEL_PRICING = {
    ...
    "deepseek-r1": {"prompt": 0.004, "completion": 0.016},
    "default": {"prompt": 0.002, "completion": 0.008},
}

_last_token_log = {}  # user_id -> last log time (防止日志风暴)


def _estimate_prompt_tokens(text: str) -> int:
    """估算prompt token数（中英文混合）"""
    import re as _re
...
def _log_token_usage(user_id: str, model_name: str, prompt_text: str, completion_text: str, latency_ms: float):
    """UP-110: 记录token消耗和费用"""
    import re as _re
    prompt_tokens = _estimate_prompt_tokens(prompt_text)
    completion_tokens = _estimate_prompt_tokens(completion_text)

    pricing = _MODEL_PRICING.get(model_name, _MODEL_PRICING["default"])
    prompt_cost = prompt_tokens / 1000 * pricing["prompt"]
    completion_cost = completion_tokens / 1000 * pricing["completion"]
    total_cost = prompt_cost + completion_cost

    # 防日志风暴：同一用户5秒内只记录一次详细日志
    now = time.time()
    last = _last_token_log.get(user_id, 0)
    if now - last > 5:
        _last_token_log[user_id] = now
        from loguru import logger as _logger
        _logger.info(
            f"[{user_id}] Token消耗: model={model_name}, "
            f"prompt={prompt_tokens}tk, completion={completion_tokens}tk, "
            f"cost=${total_cost:.6f}, latency={latency_ms:.0f}ms"
        )

    # 记录到分析系统
    try:
        from code.conversation_analytics import log_token_usage as _log_token_usage
        _log_token_usage(user_id, model_name, prompt_tokens, completion_tokens, total_cost, latency_ms)
    except Exception:
        pass
...
def call_model(prompt, temperature, model_type="main", user_id=""):
    """同步模型调用（带重试和token记录）"""
    ...
    for attempt in range(_MAX_RETRIES + 1):
        try:
            ...
            if content:
                if user_id:
                    _log_token_usage(user_id, model_name, prompt, content, (time.time() - t_start) * 1000)
                return content
            # 空回复：重试
...
async def stream_llm(system_prompt, temperature=0.5, model_type="main", user_id=""):
    """
    流式模型调用（带重试、限流保护和token记录）
    """
        ...
        try:
            async with aiohttp.ClientSession(timeout=timeout_cfg) as session:
                async with session.post(
                    ...
                    if got_content:
                        if user_id:
                            _log_token_usage(user_id, model_name, system_prompt, full_content, (time.time() - llm_start) * 1000)
                        return
                    # 空回复：重试
