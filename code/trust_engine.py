...
def can_advance_to(state, target_node: str) -> bool:
    ...
    # 确定目标阈值
    if target_node in ("icebreak", "qualify", "match_campus", "reject_qualify", "pre_assessment", "pre_training"):
...
def handle_silent_return(state, days_threshold=7):
    """沉默用户回归信任重置

    P1修复: 沉默超7天后用户主动回归，信任重置为 max(current, 50)
