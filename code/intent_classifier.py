...
INTENT_PRIORITY = {
    ...
    "competitive_inquiry": 85,
    "objection_negative_reviews": 72,
    "objection_is_scam": 72,
...
# ---- 正则快速匹配规则 ----
REGEX_RULES = [
    # 法律风险（最高优先级）- P0: 报警/投诉/录屏/举报等
    {
        "pattern": r"(报警|报警了|我报警|我要报警|投诉|举报|12315|12345|录屏|录下来|录音|证据|取证|起诉|法院|律师|工商|消协|网信办|工信部|起诉你|告你|维权)",
        "intent": "legal_threat",
        "confidence": 0.95,
    },
    # 辱骂/脏话（次高优先级）
    ...
    {
        "pattern": r"(滚蛋|滚开|他妈的|你妈的|你妈|卧槽|我去你|尼玛|马勒|傻逼|煞笔|狗屁|放屁|混蛋|王八蛋|去死吧|fuck|shit|fuc[ku]|damn|bitch|asshole|你就是骗子|忽悠人|割韭菜|坑人的|你个垃圾|你个废物|你[是这]垃圾$|骗子[！!。.]?$|垃圾[！!。.]?$|草泥马|日你妈|操你妈|龟孙子|龟儿子|狗日的|贱人|烂货|恶心人|烦死了|真恶心|什么玩意|脑残|智障|有病吧|有毛病|死骗子|臭不要脸)",
        "intent": "insult",
        "confidence": 0.95,
    },
    # 情绪优先
    ...
    # 人工接管
    {
        "pattern": r"(转人工|找人工|人工客服|换人聊|跟真人|跟人聊|不想跟机器|"
                    r"不是机器人吧|你是机器人吗|有没有真人|让真人来|叫人|找销售|找老师|找真人|找真人聊|领导|主管|经理|负责人|太套路|不像真人|我要.*真人|要.*人类.*聊)",
    ...
    # 痛点表达（注意：怕学不会/担心跟不上属于objection_learn，不在此匹配）
    {
        "pattern": r"(年纪大|年龄大|岁数|老了|在厂里|工厂|流水线|送外卖|跑滴滴|保安|"
                    r"没出路|看不到希望|不甘心|不想再|受够了|被裁|裁员|失业|找不到|"
                    r"混不下去|温水煮青蛙|怕来不及|犹豫.*转行|不敢.*辞职|被骗|上当|被坑|割韭菜|失业|交不起|付不起|扛不住|撑不下去)",
        "intent": "express_pain",
        "confidence": 0.85,
    },
    # 竞品对比（优先级高于objection_institution）
    {
        "pattern": r"(达内|黑马|传智|尚硅课|课工场|极客|优就业|蜗牛|千锋|中公|牛客|拉勾|北大青鸟|兄弟连|慕课|开课|青鸟).{0,8}(区别|不同|对比|哪个好|怎么样|靠谱|好吗|好不好|行不行|咋样)",
        "intent": "competitive_inquiry",
        "confidence": 0.88,
    },
    {
        "pattern": r"(你们|你们这).{0,5}(和|跟|比).{0,5}(区别|不同|优势|对比|哪个好|哪个强)",
        "intent": "competitive_inquiry",
        "confidence": 0.85,
    },
    {
        "pattern": r"(靠谱|正规|口碑|评价|营业执照|资质|是不是骗|是不是正规|是不是真).*(公司|机构|这里|你们)",
    ...
    # 负评差评
    {
        "pattern": r"(差评|负面|黑料|投诉|曝光|避雷|劝退|坑人|坑爹|上当了|被坑了|知乎.*看到|小红书.*看到|网上.*说|贴吧|天涯|评价.*不好|口碑.*不好|名声.*不好)",
        "intent": "objection_negative_reviews",
        "confidence": 0.85,
    },
    # 质疑套路/骗局
    {
        "pattern": r"(套路|骗子公司|骗局|是不是假的|是不是正规公司|皮包|钓鱼|诈骗|传销|是不是幌子|是不是坑|骗钱|骗培训费|假招聘|招转培|招转|虚假招聘|不靠谱|太假了|真不真|真的假的)",
        "intent": "objection_is_scam",
        "confidence": 0.85,
    },
    {
...
CONFIRM_PATTERN = re.compile(
    ...
    r"(?:嗯|可以|行的|可以的|好|好的|OK|没问题|了解|明白了|对|是的|是|对的|没错|同意|"
    r"没问题啊|好呀|行啊|OK啊|走起|来吧|行吧|嗯嗯|好嘞|妥|妥了|当然|必须的|"
    r"那行|成|中|好哦|好哒|嗯呢|嗯啊|欧了|欧克|ok的|可以啊|行哒|得|得了)"
...
- newbie: 零基础想学
- experienced: 有经验想内推
- competitive_inquiry: 问竞品对比（达内/黑马/千锋哪个好等）
- off_topic: 完全无关话题
- normal: 一般业务相关
...
def classify(msg: str, current_state: str = "", collected_slots: dict = None) -> dict:
    ...
    if regex_intent == "confirm" and len(msg) <= 3 and regex_confidence < 0.95:
        if current_state in ("qualify", "icebreak", "match_campus"):
            ...
            # P1修复: 单字确认也降级为normal，防止误推进
            # 只有包含明确确认词（好的没问题/行/可以/定/来）才保留confirm
            explicit_confirm = any(w in msg for w in ["好", "行", "可以", "定", "来", "去", "走"])
            if explicit_confirm:
                ...
                regex_intent = "normal"
                regex_confidence = 0.5
                logger.info(f"  -> 模糊确认降级: confirm -> normal (state={current_state}, msg={msg})")
