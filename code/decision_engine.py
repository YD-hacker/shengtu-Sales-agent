...
def should_end_conversation(state: dict, lead_score: int, conversation_rounds: int) -> tuple:
    ...
    if 0 < rounds_remaining <= 5:
        state["_near_round_limit"] = True
        state["_rounds_remaining"] = rounds_remaining

    # 硬上限：动态轮次必须终止（防止无限循环）
    # 但如果正在报备阶段且有未完成信息，给额外5轮
    if conversation_rounds >= max_rounds:
        current_node = state.get("current_node", "")
        if current_node == "report_info" and not (state.get("name") and state.get("phone")):
            if conversation_rounds < max_rounds + 5:
                return False, ""  # 报备阶段给额外5轮
        return True, f"对话轮次硬上限({max_rounds}轮, lead_score={lead_score})"
...
def get_round_limit_hint(state: dict, lead_score: int) -> str:
    """P1: 接近轮次上限时返回小结推进话术"""
    if not state.get("_near_round_limit"):
        return ""
    remaining = state.get("_rounds_remaining", 0)
    current_node = state.get("current_node", "icebreak")

    if current_node in ("show_fee", "invite"):
        return "咱们聊了不少了，你看这样，这周末来校区实地看看，比线上聊一百句都强，定个时间？"
    elif current_node in ("match_campus", "qualify"):
        return "咱们聊得差不多了，我帮你总结下：你的条件基本符合，下一步就是来校区看看。这周末方便吗？"
    else:
        return "咱先聊到这，有什么想了解的随时找我。周末来校区看看？"
