...
from code.memory_manager import load_state, save_state, add_history, get_history, record_active, acquire_session_lock, release_session_lock
...
from code.trust_engine import adjust_trust, init_trust, apply_daily_decay, handle_silent_return
...
from code.decision_engine import (
    ...
    should_end_conversation, should_handoff_to_human, get_round_limit_hint,
    get_conversation_pace
...
# UP-111: Prompt固定部分缓存
_prompt_prefix_cache = {}  # stage -> cached prefix string
_prompt_prefix_cache_version = 0

# UP-112: 消息频率限制
import threading as _threading
_user_msg_timestamps = {}  # user_id -> [timestamps...]
_user_msg_timestamps_lock = _threading.Lock()
_RATE_LIMIT_PER_SEC = 1     # 每秒最多1条
_RATE_LIMIT_PER_MIN = 10    # 每分钟最多10条
...
TEMPLATE_NODES = {"show_fee", "invite", "reject_qualify", "report_info", "completed", "pre_assessment", "pre_training"}
...
def _check_rate_limit(user_id: str) -> tuple:
    """UP-112: 消息频率限制检查，返回 (allowed, reason)"""
    now = time.time()
    with _user_msg_timestamps_lock:
        if user_id not in _user_msg_timestamps:
            _user_msg_timestamps[user_id] = []
        timestamps = _user_msg_timestamps[user_id]
        timestamps = [t for t in timestamps if now - t < 60]
        _user_msg_timestamps[user_id] = timestamps
        if timestamps and now - timestamps[-1] < 1.0 / _RATE_LIMIT_PER_SEC:
            return False, "消息太快，请稍后再发"
        if len(timestamps) >= _RATE_LIMIT_PER_MIN:
            return False, "消息太频繁，请1分钟后再试"
...
def _reload_config_if_needed():
    ...
    try:
        ...
        if mtime != _config_last_mtime:
            ...
            logger.info(f"CONFIG热重载完成: {CONFIG_FILE}")

        # G-008: knowledge_base.yaml 热重载
        try:
            from code import KB_FILE
            kb_mtime = os.path.getmtime(KB_FILE)
            if "_last_kb_mtime" not in dir() or kb_mtime != _reload_config_if_needed._last_kb_mtime:
                _reload_config_if_needed._last_kb_mtime = kb_mtime
                import yaml as _yaml
                with open(KB_FILE, encoding="utf-8") as _f:
                    _new_kb = _yaml.safe_load(_f)
                # 动态更新 state_machine 的 KB 引用
                from code.state_machine import KB as _state_kb
                _state_kb.clear()
                _state_kb.update(_new_kb.get("scripts", {}))
                logger.info(f"KB热重载完成: {KB_FILE}")
        except Exception as _e:
            logger.warning(f"KB热重载失败: {_e}")
...
def build_decision_prompt(user_state, intent, collected_slots,
    ...
    stage_desc = {
        ...
        "pre_assessment": "岗前考核阶段，有经验用户直接评估，通过即走快速内推通道",
        "pre_training": "岗前实训阶段，考核未通过的可选培训路径"
...
async def generate_llm_decision(user_state, intent, collected_slots,
    ...
    """让LLM自主决策生成回复，支持工具调用和对话节奏控制（UP-005）"""

    # UP-005: 对话节奏控制 - 根据pace调整回复策略
    pace_info = get_conversation_pace(user_state, lead_score)
    _pace = pace_info.get("pace", "normal")
...
async def process_message_stream(user_id, msg):
    ...
    # UP-007: trace_id全链路串联
    import uuid
    trace_id = str(uuid.uuid4())[:8]
    logger.info(f"[{user_id}] [{trace_id}] 消息处理开始")
    state = None  # type: ignore

    # UP-009: 会话级用户锁 - 防止同一用户并发处理
    if not acquire_session_lock(user_id, timeout=5):
        yield "你消息发得太快了，稍等一下，我马上回来。"
        return
    ...
    try:
        ...
        # UP-112: 消息频率限制检查
        allowed, rate_reason = _check_rate_limit(user_id)
        if not allowed:
            logger.warning(f"[{user_id}] 消息频率超限: {rate_reason}")
            yield rate_reason
            return
        ...
        # ---- 6.4 法律威胁模式清除 ----
        reconcile_words = ["好的", "算了", "不报警", "没事", "误会", "理解", "不投诉", "不举报", "继续", "说吧"]
        _has_reconcile = any(w in msg for w in reconcile_words)
        if state.get("_legal_threat_mode"):
            state["_legal_threat_rounds"] = state.get("_legal_threat_rounds", 0) + 1
            if _has_reconcile:
                state.pop("_legal_threat_mode", None)
                state.pop("_legal_threat_rounds", None)
                intent = "normal"  # 重置意图，允许正常流程
                logger.info(f"[{user_id}] 法律威胁模式: 用户和解信号，清除保护模式")
            elif state["_legal_threat_rounds"] > 5:
                state.pop("_legal_threat_mode", None)
                state.pop("_legal_threat_rounds", None)
                intent = "normal"
                logger.info(f"[{user_id}] 法律威胁模式: 超过5轮，自动清除")
        elif intent == "legal_threat" and _has_reconcile:
            # 新触发的legal_threat但含和解信号 → 不进入保护模式
            intent = "normal"
            logger.info(f"[{user_id}] 法律威胁意图含和解信号，降级为normal")
        ...
        if old_state == "qualify" and intent == "fee_intent":
            ...
            state["_fee_ask_count"] = state.get("_fee_ask_count", 0) + 1
            ...
            # P1修复: 问价>=2次时给mini报价区间，减少流失
            if state["_fee_ask_count"] >= 2:
                fee_ack = "看你这么关心费用，我先给你说个大概——1万到2万之间，具体看你的条件和方向。"
                if missing:
                    fee_ack += f"确认了{'、'.join(missing[:2])}后我给你精确报价。"
                else:
                    fee_ack += "你的条件我都了解了，费用这块马上给你详细说明。"
            elif missing:
        ...
        if tier == "layer2":
            ...
            # P0修复: 始终检查动态红线（hard_check替换后仍需过dynamic_check）
            if dynamic_check(final_text):
        ...
        if tier == "layer3":
            ...
            if old_state == "qualify":
                ...
                if not all_filled:
                    ...
                    new_state = get_next_state(old_state, state, check_intent, lead_grade)

            else:

                new_state = get_next_state(old_state, state, intent, lead_grade)
        ...
        # ---- 法律威胁意图早期处理 ----
        if intent == "legal_threat" or state.get("_legal_threat_mode"):
            state["_legal_threat_mode"] = True
            state["_legal_threat_rounds"] = state.get("_legal_threat_rounds", 0) + 1
            legal_reply = "你的反馈我已记录，我们会通过官方渠道联系你处理。如有疑问，欢迎拨打我们的官方客服电话。"
            save_state(user_id, state)
            yield legal_reply
            add_history(user_id, msg, legal_reply)
            _store_vector_memory(user_id, msg, legal_reply, state)
            flush_if_needed()
            return
        ...
        if new_state == "qualify" and old_state != "qualify" and intent == "correct_info":
            ...
            if all_filled:
                ...
                new_state = get_next_state("qualify", state, check_intent, lead_grade)
        ...
        if intent.startswith("objection_"):
            ...
            try:
                ...
                if strategy["escalate"]:
                    ...
                    # P1修复: 高价值用户强制转人工，低价值用户温和结束
                    if new_lead_score >= 60:

                        mark_user_human_active(user_id, "", 2)

                        state["current_node"] = new_state

                        save_state(user_id, state)

                        escalate_msg = "我能感觉到你还有很多顾虑，这样吧，我让我们资深的顾问直接跟你聊，他比我专业，能解答你所有问题。"

                        yield escalate_msg

                        add_history(user_id, msg, escalate_msg)

                        _store_vector_memory(user_id, msg, escalate_msg, state)
                        ...
                        end_msg = "理解，你再考虑考虑。有什么想法随时找我。"

                        yield end_msg

                        add_history(user_id, msg, end_msg)

                        _store_vector_memory(user_id, msg, end_msg, state)
        ...
        if old_state == "match_campus" and intent in ("confirm", "fee_intent"):
            ...
            from code.experiment_manager import get_experiment_template, record_exposure, assign_variant, get_experiment_for_stage
            ...
            if exp_template:
                ...
                # UP-006: 正确记录实验变体名称
                active_exp = get_experiment_for_stage("show_fee", "fee_intent")
                variant_name = assign_variant(user_id, active_exp["name"]) if active_exp else "variant"
                state["_experiment_variant"] = variant_name
                record_exposure(user_id, active_exp.get("name", "fee_show") if active_exp else "fee_show", variant_name)
        ...
        if new_state == "completed" and old_state == "report_info":
            ...
            if state.get("name") and state.get("phone"):
                ...
                # UP-006: A/B实验转化记录
                try:
                    from code.experiment_manager import record_conversion, get_experiment_for_stage
                    active_exp = get_experiment_for_stage("show_fee")
                    if active_exp:
                        variant = state.get("_experiment_variant", "control")
                        record_conversion(user_id, active_exp["name"], variant, "completed")
                        logger.info(f"[{user_id}] 实验转化记录: {active_exp['name']} variant={variant}")
                except Exception as exp_err:
                    logger.debug(f"实验转化记录失败: {exp_err}")
        ...
        # P1: 接近轮次上限时，追加小结推进话术
        round_hint = get_round_limit_hint(state, new_lead_score)
        if round_hint and intent not in ("legal_threat", "reject"):
            template = template + "\n" + round_hint
...
直接输出原话（不要加任何前缀或解释）："""
        ...
        logger.info(f"[{user_id}] [{trace_id}] 消息处理完成（耗时 {time.time() - session_start:.2f}s）")
